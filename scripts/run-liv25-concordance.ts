 ...
 * `supabase/seed/03_biomarker_thresholds.sql` (engine freeze = current main).
 * Does not mutate classifier rules or thresholds.
 *
...
function collectNotes(
  ...
  for (const key of Object.keys(values)) {
    ...
    if (alias && thresholdIds.has(alias)) {
      if (!aliasApplied.includes(`${key}→${alias}`)) {
        unused_pack_keys.push(`${key} (seed=${alias}, non mappé)`);
      }
      continue;
    }
